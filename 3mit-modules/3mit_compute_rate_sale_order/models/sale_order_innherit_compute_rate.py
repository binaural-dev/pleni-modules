...
class SaleOrderInheritCompute(models.Model):
    ...
    @api.onchange('order_id.pricelist_id', 'order_id.partner_id', 'product_id', 'product_uom_qty', 'price_unit', 'tax_id', 'name', 'order_id.order_line')
    def _onchange_price(self):
        if self.order_id.pricelist_id and self.loc_ven:
            if self.order_id.pricelist_id.currency_id.id == 3:
                ...
                if self.order_id.pricelist_id.currency_id.id != 3:
                    self.price_unit_usd = self.order_id.pricelist_id.currency_id._convert(self.price_unit, self.env.company.currency_id, self.env.company, self.order_id.date_order, False)
                    self.price_subtotal_usd = self.order_id.pricelist_id.currency_id._convert(self.price_subtotal, self.env.company.currency_id, self.env.company, self.order_id.date_order, False)
...
class SaleOrderInheritComputeTwo(models.Model):
    ...
    amount_untaxed_usd = fields.Float('Base Imponible Bs', digits=(12, 2), store=True, readonly=True)
    amount_tax_usd = fields.Float('Impuesto Bs', digits=(12, 2), store=True, readonly=True)
    amount_total_usd = fields.Float('Total Bs', digits=(12, 2), store=True, readonly=True)
    order_line = fields.One2many('sale.order.line', 'order_id', string='Order Lines',
                                 states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True,
                                 auto_join=True)

    @api.onchange('pricelist_id', 'order_line.product_id', 'order_line.product_uom_qty', 'order_line.price_unit', 'order_line.tax_id', 'order_line.name', 'amount_untaxed', 'amount_total', 'amount_tax')
    def _onchange_amount_total(self):
        ...
        if self.amount_untaxed > 0 and self.amount_total > 0 and self.loc_ven:
            ...
            if self.pricelist_id.currency_id.id != 3:
                self.amount_untaxed_usd = self.pricelist_id.currency_id._convert(self.amount_untaxed, self.env.company.currency_id, self.env.company, self.date_order, False)
                acumulado = 0
                for i in self.amount_by_group:
                    tasa = 0
                    if i[2] != 0:
                        tasa = round(i[1] / i[2], 2)
                    value = self.pricelist_id.currency_id._convert(i[2], self.env.company.currency_id, self.env.company, self.date_order, False)
