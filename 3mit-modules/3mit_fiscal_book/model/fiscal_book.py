...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
    ...
    def _get_invoice_ids(self, fb_id):
        ...
        if self.invoices_other_period() and fb_id.type == 'sale':
            facturas = self.invoices_other_period()
            inv_ids += facturas
    ...
    def invoices_other_period(self):
        invoices = []
        local_period = self.get_time_period(self.time_period)
        retentions = self.env['account.wh.iva'].search([('date_ret', '>=', local_period.get('dt_from')),
                                                        ('date_ret', '<=', local_period.get('dt_to')),
                                                        ('state', '=', 'done'),
                                                        ('company_id', '=', self.company_id.id)])
        for ret in retentions.wh_lines:
            if ret.invoice_id.invoice_date < local_period.get('dt_from') and ret.invoice_id.move_type in ['out_invoice', 'out_refund']:
                invoices.append(ret.invoice_id.id)
        invs = self.env['account.move'].search([('id', 'in', invoices)])
    ...
    def _get_issue_invoice_ids(self, fb_id):
        ...
        domain = ['|', '&', (True, '=', True), ('fb_id', '=', fb_brw.id),
                  ...
                  ('date', '<=', local_period.get('dt_to')),
                  ('move_type', 'in', inv_type), ('state', 'in', inv_state), ('company_id', '=', self.company_id.id)]
    ...
    def update_book_lines(self, fb_id):
        ...
        for inv_brw in self.browse(fb_id).issue_invoice_ids:
            ...
            if (inv_brw.wh_iva_id.state == 'draft' or (inv_brw.wh_iva_id.state == 'done' and inv_brw.wh_iva_id.date_ret > local_period.get('dt_to'))) and (inv_brw.state == 'posted' or inv_brw.state == 'cancel') and inv_brw.sin_cred == False:
  ...
  #          for iwdl_brw in iwdl_ids:
        ...
        if inv_iva:
                ...
                for iwdl_brw in iwdl_ids:
                    # if iwdl_brw.type == 'in_invoice':
                        ...
                        is_old = False
                        if fb_brw.type == 'sale':
                            is_old = iwdl_brw.invoice_id.invoice_date < local_period.get('dt_from') if iwdl_brw.invoice_id.invoice_date else iwdl_brw.invoice_id.date < local_period.get('dt_from')
      ...
      #      if not self.browse(fb_id).iwdl_ids and iwdl_id == False:
                ...
                is_old = False
                if fb_brw.type == 'sale':
                    is_old = inv_otro.invoice_date < local_period.get('dt_from') if inv_otro.invoice_date else inv_otro.date < local_period.get('dt_from')
  ...
  #      ait_obj = self.env['account.invoice.tax']
        ...
        for fbl_brw in self.fbl_ids:
            ...
            if fbl_brw.iwdl_id.invoice_id and not fbl_brw.is_old:
                for ait in fbl_brw.iwdl_id.tax_line:
                    base_amount += ait.base * sign
                    tax_amount += ait.amount * sign
            if fbl_brw.invoice_id and fbl_brw.invoice_id.state != 'cancel' and not fbl_brw.is_old:
                tasa = 1
 ...
 #       ait_obj = self.env['account.invoice.tax']
        ...
        for fbl in fiscal_book.fbl_ids:
            ...
            if fbl.iwdl_id.invoice_id and not fbl.is_old:
            ...
            if fbl.invoice_id and fbl.invoice_id.state != 'cancel' and not fbl.is_old:
                tasa = 1
                if fbl.invoice_id.currency_id.name == "USD":
                    tasa = self.obtener_tasa(fbl.invoice_id)
                fiscal_book = self.browse(fb_id)
                ...
                for line in fbl.invoice_id.invoice_line_ids:
                    ...
                    if busq:
                        if rate == 0:
                            ...
                            base = (line.price_subtotal) * tasa
                            amount = (line.price_subtotal) * rate * tasa

                        if (rate == 0 or (rate*100) > 1) and line.price_total > 0:
                            amount_field_data['total_with_iva'] += (line.price_subtotal * sign * tasa) + (line.price_subtotal * rate * tasa)
                            if busq == 'sdcf':
                                amount_field_data['vat_sdcf'] += base * sign
                            if busq == 'exento':
                                amount_field_data['vat_exempt'] += base * sign
            ...
            if fbl.invoice_id and fbl.invoice_id.state == 'cancel' or fbl.is_old:
                amount_field_data = {'total_with_iva':
                                         0.0,
                                     'vat_sdcf': 0.0, 'vat_exempt': 0.0, 'vat_general_base': 0.0, }
                fbl.write(amount_field_data)
            ...
            if fbl_brw.iwdl_id.invoice_id and not fbl_brw.is_old:
               ...
               for line in fbl_brw.iwdl_id.invoice_id.invoice_line_ids:
                   ...
                   if line.currency_id.name == "USD":
                       tasa = self.obtener_tasa(fbl_brw.iwdl_id.invoice_id)
                   for field_name in field_names:
                        field_tax, field_amount = field_name[4:].split('_')
                        base = line.price_subtotal * tasa
                        tax_amount = line.price_subtotal * rate * tasa
                        if busq:
    ...
    def get_transaction_type(self, fb_id, inv_id):
        ...
        if fb_brw.type == 'purchase':
            ...
            return 'tp' if inv_brw.partner_id.wh_iva_agent else 'ntp'
    ...
    def obtener_tasa(self, invoice):
        if invoice.currency_bs_rate:
            return invoice.currency_bs_rate
        else:
            fecha = invoice.date
            tasa_id = invoice.currency_id
            fecha = fields.datetime.combine(fecha, fields.datetime.max.time())
...
class FiscalBookLines(models.Model):
 ...
 #   _parent_store = "True"
    is_old = fields.Boolean(default=False)
...
class AdjustmentBookLine(models.Model):
    ...
    amount_untaxed_n = fields.Float('Amount Untaxed', #digits='Account', required=True,
                                    help="Amount untaxed for national operations")
    percent_with_vat_n = fields.Float('VAT Withholding (%)', #digits='Account', required=True,
                                      help="VAT percent (%) for national operations")
    amount_with_vat_n = fields.Float('VAT Withholding Amount', # digits='Account', required=True,
                                     help="Percent(%) VAT for national operations")
    amount_untaxed_i = fields.Float('Amount Untaxed', # digits='Account', required=True,
                                    help="Amount untaxed for international operations")
    percent_with_vat_i = fields.Float('VAT Withholding (%)', #digits='Account', required=True,
                                      help="VAT percent (%) for international operations")
    amount_with_vat_i = fields.Float('VAT Withholding Amount', # digits='Account', required=True,
