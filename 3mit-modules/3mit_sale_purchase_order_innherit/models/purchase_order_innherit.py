    ...
    def write(self, vals):
        ...
        if vals.get('partner_id') and self.loc_ven:
                ...
                if (partner_obj.company_type == 'person' and not partner_obj.identification_id):
                    raise UserError('El Proveedor no posee Documento Fiscal, por favor diríjase a la configuación de %s, y realice el registro correctamente para poder continuar. #1' % str(partner_obj.name))
                if (partner_obj.company_type == 'company'):
                    if (partner_obj.people_type_company == 'pjdo' and not partner_obj.rif):
                        raise UserError('El Proveedor no posee Documento Fiscal, por favor diríjase a la configuación de %s, y realice el registro correctamente para poder continuar, #2' % str(partner_obj.name))
