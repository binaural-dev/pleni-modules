...
odoo.define('website_uom_select_suggetion_spt.WebsiteUOM', function (require) {
  "use strict";
  ...
  $(document).ready(function () {
    $('#uom_name').html($("#uom_id")[0].options[$("#uom_id")[0].selectedIndex].text);
  });
  $(document).on('change', '#uom_id, input.js_variant_change', function (ev) {
    // var vals = {
    //     "uom": $("#uom_id").val(),
    //     'product_id': $('.product_id').val(),
    // }
    ProductConfiguratorMixin.onChangeVariant(ev); // ajax.jsonRpc("/shop/onchange", "call", vals).then(function(rec_data){
    //     debugger;
    //     $('#uom_name').html(rec_data['uom']);
    //     $('h4.oe_price_h4').find('.oe_currency_value').html(rec_data['price'].toFixed(2));
    //     // $('.availability_messages.o_not_editable').find('.text-success').html(rec_data['uom'] + ' availabel');
  ...
  ProductConfiguratorMixin._onChangeCombinationStock = function (ev, $parent, combination) {
    var product_id = 0; // needed for list view of variants
