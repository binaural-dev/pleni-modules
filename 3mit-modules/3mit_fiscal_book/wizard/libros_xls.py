...
class FiscalBookWizard(models.TransientModel):
    ...
    def xls_compra(self):
        ...
        row += 1
        writer.write_merge(row, row, 1, 5, 'RIF.: ' + libro_id.company_id.rif, header_content_style_left)
        row += 1
        street = street2 = ''
        ...
        for datos in datos_compras:
            ...
            writer.write_merge(row, row, 12, 13, datos.get('credit_affected'), sub_header_style_1)
            writer.write_merge(row, row, 14, 15, datos.get('debit_affected'), sub_header_style_1)
    ...
    def xls_ventas(self):
        ...
        writer.write_merge(row, row, 1, 55, 'Desde: ' + date_start + ' Hasta: ' + date_end, header_content_style)
        row += 1
        writer.write_merge(row, row, 1, 7, 'RETENCIONES DE OTROS PERÍODOS APLICADAS EN EL PERÍODO ACTUAL', header_content_style)
        row += 1
        #aca va pre encabezado
        ...
        for datos in docs1:
            ...
            if type(datos.get('date_wh_number')) != bool:
                writer.write_merge(row, row, 50, 51, datetime.strftime(datos.get('date_wh_number'), format_new), sub_header_style_1)
            else:
                writer.write_merge(row, row, 50, 51, datos.get('date_wh_number'),
                                   sub_header_style_1)
        row += 2
        writer.write_merge(row, row, 20, 21, "Totales", sub_header_style)
        writer.write_merge(row, row, 22, 23, 0.00, sub_header_style)
        writer.write_merge(row, row, 24, 25, 0.00, sub_header_style)
        writer.write_merge(row, row, 26, 27, 0.00, sub_header_style)
        writer.write_merge(row, row, 28, 29, 0.00, sub_header_style)
        writer.write_merge(row, row, 30, 31, "", sub_header_style)
        writer.write_merge(row, row, 32, 33, 0.00, sub_header_style)
        writer.write_merge(row, row, 34, 35, 0.00, sub_header_style)
        writer.write_merge(row, row, 36, 37, "", sub_header_style)
        writer.write_merge(row, row, 38, 39, 0.00, sub_header_style)
        writer.write_merge(row, row, 40, 41, 0.00, sub_header_style)
        writer.write_merge(row, row, 42, 43, "", sub_header_style)
        writer.write_merge(row, row, 44, 45, 0.00, sub_header_style)
        writer.write_merge(row, row, 46, 47, suma_get_wh_vat_old, sub_header_style)
        ...
        for datos in docs:
            ...
            writer.write_merge(row, row, 48, 49, datos.get('wh_number'), sub_header_style_1)
            if type(datos.get('date_wh_number')) != bool:
                writer.write_merge(row, row, 50, 51, datetime.strftime(datos.get('date_wh_number'), format_new),
                                   sub_header_style_1)
            else:
                writer.write_merge(row, row, 50, 51, datos.get('date_wh_number'),
                                   sub_header_style_1)
