...
class KitsVariantController(VariantController):
    ...
    def get_combination_info(self, product_template_id, product_id, combination, add_qty, pricelist_id, **kw):
        ...
        if res.get('virtual_available') and product.uom_id.id != uom.id:
            ...
            virtual_available = int(product.uom_id._compute_quantity(now_in_stock, uom))
            res.update({
    ...
    def _update_combination_vals(self, request, vals, uom, add_qty, pricelist_id):
        pricelist_items = request.env['product.pricelist.item'].sudo().search([('pricelist_id','=',pricelist_id.id),'|',('product_tmpl_id','=',vals['product_template_id']),('product_id','=',vals['product_id'])], order= 'min_quantity asc')
        base_oum = request.env['uom.uom'].sudo().search([('category_id','=',uom.category_id.id),('uom_type','=','reference')])
        ...
        real_list_price = max([item.fixed_price for item in pricelist_items])
