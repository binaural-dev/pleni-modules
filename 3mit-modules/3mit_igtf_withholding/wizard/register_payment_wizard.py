    ...
    def register_IGTF_payment(self):
        if self._context.get('active_model') == 'account.move':
            ...
            move_advance_ = {
                ...
                'move_id': move_id.id,
                'credit': to_pay if move.move_type == 'out_invoice' else 0.0,
                'debit': 0.0 if move.move_type == 'out_invoice' else to_pay,
            }
            if self.currency_id.id != self.env.company.currency_id.id:
                sign = -1 if move.move_type == 'out_invoice' else 1
                move_advance_['currency_id'] = self.currency_id.id
                move_advance_['amount_currency'] = sign * to_pay_conversion
            ...
            asiento['credit'] = 0.0 if move.move_type == 'out_invoice' else to_pay
            asiento['debit'] = to_pay if move.move_type == 'out_invoice' else 0.0
            if self.currency_id.id != self.env.company.currency_id.id:
                sign = 1 if move.move_type == 'out_invoice' else -1
                asiento['currency_id'] = self.currency_id.id
                asiento['amount_currency'] = sign * to_pay_conversion
