odoo.define('3mit_new_fields_in_website_payment.website_sale_delivery_date', function (require) {
    'use strict';
     ...
     */
     payEvent: function (ev) {
        ev.preventDefault();
        // Inicio de comprobacion de delivery
        const client_notes = $("textarea[name='client_notes']").val()
        const delivery_date = $("input[name='delivery_date']").val()
        const delivery_hour = $("select[name='delivery_hour']").val()
        if (!delivery_date) {
            this.displayError(
            _t('No ha seleccionado una fecha de entrega'),
            _t('Por favor, selececione una fecha de entrega.')
            );
            return
        }
        const new_delivery_date = new Date(delivery_date)
        const todayDate = new Date()
        const today = todayDate.getFullYear()+'-'+((todayDate.getMonth()+1)/10 < 1? '0' : '')+(todayDate.getMonth()+1)+'-'+todayDate.getDate()
        const newTodayDate = new Date(today)
        if (new_delivery_date.getTime()+4 * 60 * 60000 < newTodayDate.getTime()) {
            console.log(today)
            $("input[name='delivery_date']").val(today)
            this.displayError(
            _t('Erro en la fecha de entrega'),
            _t('La fecha de entrega no puede ser antes de hoy.')
            );
            return
        }
        // fin de comprobacion de delivery
        var form = this.el;
