...
class TxtIva(models.Model):
    ...
    def get_document_affected(self, txt_line):
        ...
        """
        number = '0'
        if txt_line.invoice_id.move_type in ['out_refund', 'in_refund'] or txt_line.invoice_id.name.find("ND") != -1 or txt_line.invoice_id.name.find("nd") != -1\
        ...
        elif txt_line.invoice_id.debit_origin_id:
            number = txt_line.invoice_id.debit_origin_id.supplier_invoice_number
    ...
    def get_type_document(self, txt_line):
        ...
        """
        inv_type = '03'
        if txt_line.invoice_id.move_type in ['out_invoice', 'in_invoice'] and txt_line.invoice_id.partner_id.people_type_company != 'pjnd' and not txt_line.invoice_id.debit_origin_id :
            inv_type = '01'
        elif txt_line.invoice_id.move_type in ['out_invoice', 'in_invoice'] and \
                txt_line.invoice_id.debit_origin_id:
            inv_type = '02'
            ...
            inv_type = '05'
        elif txt_line.invoice_id.move_type in ['out_invoice', 'in_invoice'] and txt_line.invoice_id.partner_id.people_type_company != 'pjnd' :
            inv_type = '01'
    ...
    def get_amount_line(self, txt_line, amount_exempt):
        ...
        alic_porc = 0
        busq = self.env['account.tax'].search([('name', '=', txt_line.tax_wh_iva_id), ('company_id', '=', self.company_id.id)])
        if busq:
    ...
    def generate_txt(self):
        ...
        for txt in self:
            ...
            for txt_line in txt.txt_ids:
                ...
                suma_iva = abs(txt_line.untaxed)
                for line in txt_line.invoice_id.wh_iva_id.wh_lines.tax_line:
                    suma_iva = suma_iva + abs(line.amount)
                if txt_line.voucher_id == txt_line.invoice_id.wh_iva_id:
                    amount_total11 = txt_line.invoice_id.amount_untaxed
                    for i in txt_line.invoice_id.amount_by_group:
                        if i[0].find('IVA 16%') != -1 or i[0].find('IVA 8%') != -1 or i[0].find('IVA 31%') != -1:
                            amount_total11 = amount_total11 + i[1]
                    amount_total2 = str(round(amount_total11, 2))
                    amount_untaxed = txt_line.untaxed - txt_line.amount_sdcf
                else:
                    amount_total2 = str(round(amount_total, 2))
                    amount_untaxed = amount_untaxed - txt_line.amount_sdcf
                ...
                amount_total2 = round(suma_iva, 2)
