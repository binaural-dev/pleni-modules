...
class WebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if not ppg:
            ppg = 24
            #request.env['website'].get_current_website().shop_ppg or 20
        ...
        search_product = Product.search(domain, order='times_sold desc')
        ...
        order_check = self._get_search_order(post)

        if 'website_sequence ASC' in order_check:
            order_check = order_check.replace('website_sequence ASC', 'name asc')
        ...
        prod_lst = Product.search(domain, limit=False, order='times_sold desc')
        ...
        values = {
            ...
            'bins': TableCompute().process(products, 24, 6),
            'ppg': 24,
            'ppr': 6,
            'tags':tags,
            'tag_list':tag_list,
