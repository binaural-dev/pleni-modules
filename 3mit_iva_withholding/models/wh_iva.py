...
class AccountWhIvaLineTax(models.Model):
    ...
    base = fields.Float(
        string='Base de la factura', digit='Withhold',
    ...
    amount_ret = fields.Float(
        ...
        store=True, compute='_get_amount_ret', digits='Withhold', inverse='_set_amount_ret',
    ...
    def _get_base_amount(self):
        ...
        for record in self:
            ...
            record.amount_original_currency = round(record.base, 2)
            ...
            amount = round(record.amount_original_currency * (record.alicuota / 100), 2)
            record.amount = f_xc(amount)

    def _get_only_amount(self):
        for record in self:
            if record.move_id.currency_bs_rate and record.move_id.currency_id.id != self.env.company.currency_id.id:
                amount = round(record.amount_original_currency * (record.alicuota / 100),2)
                record.amount = round(amount * self.move_id.currency_bs_rate, 2)
            else:
                f_xc = self.env['l10n.ut'].sxc(
                    record.move_id.currency_id.id,
                    record.move_id.company_id.currency_id.id,
    ...
    def _get_amount_ret(self):
        ...
        for record in self:
            ...
            # WITHHOLDING
            if record.move_id.currency_bs_rate and record.move_id.currency_id.id != self.env.company.currency_id.id:
                amount_ret = round((round(record.amount_original_currency * (record.alicuota / 100),
                                          2)) * record.wh_vat_line_id.wh_iva_rate / 100.0, 2)
                record.amount_ret = round(amount_ret * record.move_id.currency_bs_rate, 2)
            else:
                f_xc = self.env['l10n.ut'].sxc(
                    record.move_id.currency_id.id,
                    record.move_id.company_id.currency_id.id,
                    record.wh_vat_line_id.retention_id.date)
                amount_ret = round((round(record.amount_original_currency * (record.alicuota/100), 2)) * record.wh_vat_line_id.wh_iva_rate / 100.0, 2)
                record.amount_ret = f_xc(amount_ret)
...
class AccountWhIvaLine(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', related='retention_id.partner_id')
    name = fields.Char(
    ...
    def load_taxes(self):
        ...
        for rec in self:
            if rec.invoice_id:
                if rec.retention_id.type in ('in_invoice', 'in_refund', 'in_debit'):
                    rate = abs(rec.invoice_id.partner_id.tax_percentage.amount)
                else:
                ...
                for line_ids in rec.invoice_id.invoice_line_ids:
                    ...
                    if len(line_ids.tax_ids) > 1:
                        taxxx = list(filter(lambda tax: tax.type_tax == 'iva' and tax.amount > 0, line_ids.tax_ids))
                    else:
                    ...
                    if taxd:
                        awilt.create({'wh_vat_line_id': rec.id,
                                      ...
                                      'base': monto_subtotal,
                                      'amount': monto_subtotal * (tax.amount/100)
                                      })
...
class AccountWhIva(models.Model):
        ...
        for ret in self:
            if ret.state == 'done':
                for ret_line in ret.wh_lines:
                    if ret_line.move_id:
                        ret_line.move_id._reverse_moves(
                            [{'date': fields.Date.today(), 'ref': _('Reversal of %s') % ret_line.move_id.name}],
                            cancel=True)
    ...
    def _get_valid_wh(self, amount_ret, amount, wh_iva_rate,
        ...
        if amount != 0 and amount_ret != 0:
            amount_total = round(amount * (wh_iva_rate - offset) / 100.0, 2)
            amount_total2 = round(amount * (wh_iva_rate + offset) / 100.0, 2)
        else:
            amount_total = 0
            amount_total2 = 0
        return (amount_ret >= float(amount_total) and
                amount_ret <= float(amount_total2))
    ...
    def _check_partner(self):
        ...
        """
        if self.partner_id:
            if not self.partner_id.purchase_journal_id:
                raise exceptions.ValidationError(
                    _('Asigne un Diario de IVA al proveedor.'))
            else:
                self.journal_id = self.partner_id.purchase_journal_id
            if not self.partner_id.purchase_journal_id.default_iva_account:
                raise exceptions.ValidationError(
                    _('Asigne un una cuenta al Diario de retención de IVA compras.'))
            else:
                self.account_id = self.partner_id.purchase_journal_id.default_iva_account
    ...
    def clear_wh_lines(self):
        ...
        if self.ids:
            ...
            if invoice: invoice.write({'wh_iva': False})
    ...
    def confirm_check(self):
        ...
        if (not self.check_wh_lines() or
            ...
            if self.type not in ('in_invoice', 'in_refund', 'in_debit'):
            ...
            if ret.wh_lines:
                ...
                for line in ret.wh_lines:
                    ...
                    if (line.invoice_id.currency_id.id != line.invoice_id.company_id.currency_id.id):
                        f_xc = ret_move.currency_bs_rate
                        for ml in self.env['account.move.line'].search([('move_id', '=', ret_move.id)]):
                            ml.write({
                                'currency_id': line.invoice_id.currency_id.id})

                            if ml.credit:
                                amount_currency_new_value = 0
                                if f_xc != 0:
                                    amount_currency_new_value = (ml.credit / f_xc) * -1
                                ml.write({
                                    'amount_currency': amount_currency_new_value})

                            elif ml.debit:
                                amount_currency_new_value = 0
                                if f_xc != 0:
                                    amount_currency_new_value = (ml.debit / f_xc)
                                ml.write({
                                    'amount_currency': amount_currency_new_value})
                    ret_move.post()
    ...
    def _get_sequence_code(self):
        ...
        local_number = ir_sequence.next_by_code(sequence_code)
        if local_number and self.date_ret:
            account_month = str(self.date_ret).split('-')[1]
            account_year = str(self.date_ret).split('-')[0]
    ...
    def _amount_ret_all(self):
        ...
        for rec in self:
            ...
            if rec.create_date:
                if rec.wh_lines:
                    rec.total_tax_ret = sum(line.amount_tax_ret for line in rec.wh_lines)
                    rec.amount_base_ret = sum(line.base_ret for line in rec.wh_lines)
